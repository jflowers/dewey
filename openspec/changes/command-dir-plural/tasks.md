...
- [ ] 2.1 In `cli_test.go`, update all 4 test functions (`TestInitCmd_ScaffoldsSlashCommands`, `TestInitCmd_SkipsExistingSlashCommands`, `TestInitCmd_NoOpenCodeDir`, `TestInitCmd_ReInitScaffoldsNewCommands`) to use `.opencode/commands/` instead of `.opencode/command/`. Also fix pre-existing gap: add `dewey-curate.md` to the assertion list in `TestInitCmd_ScaffoldsSlashCommands` (currently checks 5 of 6 commands)
- [ ] 2.2 In `cli_test.go`, add a new test `TestInitCmd_MigratesOldCommandDir` that verifies migration: create old dir with at least 2 Dewey slash command files, run init, assert all files moved to new dir, assert old dir removed when empty, assert all `deweySlashCommands` entries exist in `.opencode/commands/` after init (scaffolding proceeds after migration)
- [ ] 2.3 In `cli_test.go`, add a test `TestInitCmd_MigrationSkipsExistingInNewDir` that verifies: old dir has a file with distinct content, new dir already has the same filename with different content, run init, assert new dir file retains its original content, old copy is removed
- [ ] 2.4 In `cli_test.go`, add a test `TestInitCmd_MigrationKeepsOldDirWithNonDeweyFiles` that verifies: create old dir with a Dewey file AND a non-Dewey file (e.g., `speckit.plan.md`), run init, assert Dewey file migrated to new dir, assert non-Dewey file untouched in old dir, assert old dir still exists

## 3. Verification

- [ ] 3.1 Run `go build ./...` — must pass
- [ ] 3.2 Run `go vet ./...` — must pass
- [ ] 3.3 Run `go test -race -count=1 ./...` — must pass
- [ ] 3.4 Verify constitution alignment: Composability First (Dewey only migrates its own commands), Observable Quality (migration logged), Testability (all 8 spec scenarios covered by tests)
